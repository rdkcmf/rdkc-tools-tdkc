...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>sysutil_WEBUI_WEBRTCDemo_LiveStreamPlay</name>
  <primitive_test_id/>
  <primitive_test_name>ExecuteCmd_TDKC</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test script to start playing live stream and verify whether video is playing or not using debug logs</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_sysutil_09</test_case_id>
    <test_objective>Test script to start playing live stream and verify whether video is playing or not using debug logs</test_objective>
...
11.revert wifi and rms conf files and unload the module</automation_approch>
    ...
    <test_script>sysutil_WEBUI_WEBRTCDemo_LiveStreamPlay</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'sysutil_WEBUI_WEBRTCDemo_LiveStreamPlay');
        ...
        if status == "SUCCESS":
            ...
            if status == "SUCCESS":
                ...
                if status == "SUCCESS":
                    ...
                    tdkTestObj.setResultStatus("FAILURE");
                    print "ACTUAL RESULT : Playing video stream failed"
                    print "[TEST EXECUTION RESULT ] : FAILURE\n"
            else:
                tdkTestObj.setResultStatus("FAILURE");

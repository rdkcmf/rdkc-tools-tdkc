...
<xml>
  ...
  <name>sysutil_CheckNetworkConfig_Persistent_AfterReboot</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test script to update network configurations in wpa_supplicant.conf and check whether the configurations are retained after device reboot</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_sysutil_05</test_case_id>
    <test_objective>Test script to update network configurations in wpa_supplicant.conf and check whether the configurations are retained after device reboot</test_objective>
...
2.If the load module status is success, check whether network configurations in wpa_supplicant.conf is persistent.
3.Invoke ExecuteCmd function to execute "updateNetworkConfig.sh" script with ssid &amp; psk to be set as its arguments.
4.Reboot the device.
5.When the device comes up, check whether network configurations set is available in wpa_supplicant.conf.
6.Invoke ExecuteCmd function to execute "updateNetworkConfig.sh" script to revert the network configurations set
7.Update the test result as success / failure based on network configurations persistent status.</automation_approch>
    <expected_output>wpa_supplicant network configurations should be retained after device reboot</expected_output>
    <priority>High</priority>
    <test_stub_interface>systemutil_tdkc</test_stub_interface>
    <test_script>sysutil_CheckNetworkConfig_Persistent_AfterReboot</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'sysutil_CheckNetworkConfig_Persistent_AfterReboot');
...
if "SUCCESS" in result.upper() :
    ...
    if expectedresult in actualresult:
        ...
        print "\nTEST STEP 2 : To get wpa_supplicant.conf after updating network config details"
        print "EXPECTED RESULT : updated wpa_supplicant.conf should be obtained"
        tdkTestObj = obj.createTestStep('ExecuteCmd_TDKC');
        expectedresult = "SUCCESS"
        ...
        if expectedresult in actualresult:
            ...
            print "ACTUAL RESULT  : Command Execution to get updated wpa_supplicant.conf success"
            print "Value Returned : "
            print(cmdOutput)

            print "\nTEST STEP 3 : To check whether network configurations in wpa_supplicant.conf retained"
            print "EXPECTED RESULT : Network configurations set should be available"
            if ssid in cmdOutput and psk in cmdOutput:
                print "ACTUAL RESULT   : network configurations retained"
                print "[TEST EXECUTION RESULT] : SUCCESS\n"
                tdkTestObj.setResultStatus("SUCCESS");
            else:
                print "ACTUAL RESULT   : network configurations not retained"
                print "[TEST EXECUTION RESULT] : FAILURE\n"
                tdkTestObj.setResultStatus("FAILURE");
        else:
            print "ACTUAL RESULT  : Command execution to get updated wpa_supplicant.conf Failed"
